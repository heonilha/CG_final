...
GLint g_lightPosLoc = -1;
GLint g_viewPosLoc = -1;
...
void drawGrid(glm::mat4 view, glm::mat4 projection) {
    ...
    // === 조명 & 카메라 위치 전달 (플레이어 주변만 밝게) ===
// 플레이어 좌표 기준으로 약간 위에 조명을 둠
    glm::vec3 lightPos(g_playerPosX, 1.5f, g_playerPosZ);
    glUniform3fv(g_lightPosLoc, 1, glm::value_ptr(lightPos));

    // 카메라 위치 (이미 전역으로 관리 중)
    glUniform3fv(g_viewPosLoc, 1, glm::value_ptr(g_cameraPos));
    ...
    for (int i = 0; i < g_gridHeight; ++i) {
        for (int j = 0; j < g_gridWidth; ++j) {
            ...
            if (g_isMinimapView) {
                glUniform1i(glGetUniformLocation(g_shaderProgram, "lightingMode"), 0);
                // 미니맵용 색
                if (g_maze[i][j] == WALL) {
                    // 예: 벽 = 흰색, 바닥 = 검정
                ...
                glUniform1i(glGetUniformLocation(g_shaderProgram, "lightingMode"), 1);
                // 메인 화면용 색(취향대로)
                if (g_maze[i][j] == WALL) {
...
void handlePlayerInput(float deltaTime) {
    float yaw = glm::radians(g_playerAngleY);

    // 팩맨이 바라보는 "앞" 방향 (forward)
    glm::vec3 forward(sin(yaw), 0.0f, cos(yaw));

    // forward 기준 오른쪽(right)
    glm::vec3 right(forward.z, 0.0f, -forward.x);
    glm::vec3 moveVector(0.0f, 0.0f, 0.0f);

    if (g_specialKeyStates[GLUT_KEY_UP] || g_keyStates['w'] || g_keyStates['W'])
        moveVector += forward;      // "앞" → 팩맨이 바라보는 방향
    if (g_specialKeyStates[GLUT_KEY_DOWN] || g_keyStates['s'] || g_keyStates['S'])
        moveVector -= forward;      // "뒤"

    if (g_specialKeyStates[GLUT_KEY_LEFT] || g_keyStates['a'] || g_keyStates['A'])
        moveVector -= right;        // "왼쪽"
    if (g_specialKeyStates[GLUT_KEY_RIGHT] || g_keyStates['d'] || g_keyStates['D'])
        moveVector += right;        // "오른쪽"
